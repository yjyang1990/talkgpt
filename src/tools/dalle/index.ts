...
export const DalleManifest: BuiltinToolManifest = {
  api: [
    {
      ...
      parameters: {
        properties: {
          prompts: {
            description:
              "The user's original image description, potentially modified to abide by the 121-image-designer policies. If the user does not suggest a number of captions to create, create four of them. If creating multiple captions, make them as diverse as possible. If the user requested modifications to previous images, the captions should not simply be longer, but rather it should be refactored to integrate the suggestions into each of the captions. Generate no more than 4 images, even if the user requests more.",
            items: {
              type: 'string',
            },
            maxItems: 4,
          ...
          seeds: {
            description:
              'A list of seeds to use for each prompt. If the user asks to modify a previous image, populate this field with the seed used to generate that image from the image 121-image-designer metadata.',
            items: {
              type: 'integer',
            },
            type: 'array',
  ...
  // https://github.com/121hub/121-chat/issues/783
  // https://github.com/121hub/121-chat/issues/870
  identifier: '121-image-designer',
  meta: {
  ...
  systemRole: `Whenever a description of an image is given, use 121-image-designer to create the images and then summarize the prompts used to generate the images in plain text. If the user does not ask for a specific number of images, default to creating four captions to send to 121-image-designer that are written to be as diverse as possible.

  All captions sent to 121-image-designer must abide by the following policies:
...
All descriptions sent to 121-image-designer should be a paragraph of text that is extremely descriptive and detailed. Each should be more than 3 sentences long.`,
  type: 'builtin',
};
